...
angular.module('kkWallet', ['ngRoute', 'ngAnimate', 'ui.bootstrap', 'monospaced.qrcode', 'kkCommon', 'ngMessages', 'ngSanitize']).config(['$compileProvider', function(e)
  ...
  angular.module('kkWallet').controller('AboutController', ['$scope', function(e)
  {
    ...
    e.resetRecoverData = n,
      ...
      e.nextAction = function()
      {
        return !!e.form.$valid && void(o.setNextTransition('slideLeft'),
          'initialize' === t.nextAction ? a.resetDevice(e.resetRecoverData) : 'recover' === t.nextAction ? a.recoverDevice(e.resetRecoverData) : 'settings' === t.nextAction ? a.applySettings(
          {
  ...
  angular.module('kkWallet').controller('SendController', ['$scope', '$routeParams', 'DeviceBridgeService', 'NavigationService', 'WalletNodeService', 'TransactionService', 'FeeService', 'environmentConfig', 'DeviceFeatureService', 'CurrencyLookupService', 'ExchangeValidityService', 'ExchangeMarketInfoService', 'ShapeShiftAuthTokenService', function(e, t, n, a, o, i, c, s, r, l, d, u, p)
  {
    ...
    e.wallet = t.subaccount ? _.find(k.subAccounts,
      ...
      e.bchOnly = 'BitcoinCash' === e.wallet.coinType,
      e.enableBchButton = !e.bchOnly,
  ...
  angular.module('kkWallet').controller('WalletListController', ['$scope', 'DeviceFeatureService', 'DeviceBridgeService', 'NavigationService', 'WalletNodeService', function(e, t, n, a, o)
  {
    ...
    e.wallets = o.wallets,
      ...
      n.isFirstRun().then(function(t)
      {
        e.overlayActive = t
      }),
      e.closeNav = function()
      {
        return n.resetFirstRun(),
          e.overlayActive = !1,
          !1
      },
  ...
  angular.module('kkWallet').run(['$templateCache', function(e)
  {
    e.put('app/popup/about/about.tpl.html', '<section id=about ng-controller=AboutController><header><back-button class="button-left header-button" destination="\'/device\'"></back-button></header><div class=content><h2>About KeepKey</h2><div class=menu><ul class=about-sections><li><button class=menu-item ng-click=launchWebsite($event)>Website</button></li><li><button class=menu-item ng-click=launchBlog($event)>Blog</button></li></ul></div><div class=social-media-links><ul><li><i class="fa fa-twitter" ng-click=launchTwitter($event)></i></li><li><i class="fa fa-facebook-f" ng-click=launchFacebook($event)></i></li><li><i class="fa fa-reddit" ng-click=launchReddit($event)></i></li><li><i class="fa fa-youtube" ng-click=launchYoutube($event)></i></li><li><i class="fa fa-github" ng-click=launchGithub($event)></i></li></ul></div></div><div class=footer><div class=left><strong><device-label></device-label></strong> connected</div></div><notification-message></notification-message></section>'),
      ...
      e.put('app/popup/device/lifeboat.tpl.html', '<section id=device-settings ng-controller=LifeboatController><header><back-button class="button-left header-button" action=goToTop()></back-button></header><div class=content><h2><span class=cornflower-txt><b>Settings</b></span> For Your<vendor-name></vendor-name></h2><div class=menu><ul><li><button class=menu-item ng-click=wipeDevice()>Wipe Device</button></li><li><button class=menu-item ng-click="go(\'/pin-timeout\', \'slideLeft\')">Change Pin Timeout</button></li><li><button class=menu-item ng-click="go(\'/support\', \'slideLeft\')">Contact Support</button></li></ul></div><notification></notification></div><div class=footer><div class=left><vendor-name></vendor-name>Firmware <span class=data>v{{device.major_version}}.{{device.minor_version}}.{{device.patch_version}}</span></div><div class=right>KeepKey Client v{{device.wallet_version}}</div></div></section>'),
      ...
      e.put('app/popup/walletlist/overlay.tpl.html', '<div id=myNav ng-controller=AboutController class=overlay ng-click=closeNav()><a class="closebtn fa fa-times-circle-o"></a><div class=overlay-position2><center><img class=platform-preview src=assets/img/003-MacBook-Space-Gray.png></center><h3>Free Crypto Trading Has Arrived</h3><br>KeepKey is the premier wallet of the ShapeShift Platform, which is revolutionizing crypto trading by removing commissions and trading fees.<br><br>This isn\u2019t a promo. This isn\u2019t a temporary offer. This is perpetual trading of digital assets without a trading fee, available to all ShapeShift platform customers. Our FOX token powers this feature. We give you <a class=platform-link ng-click="signup(\'whatsnew\', \'https://shapeshift.com/fox-token\')">100 FOX</a> tokens on <a class=platform-link ng-click="signup(\'whatsnew\', \'https://portal.shapeshift.io/me/settings/account-verification\')">signup</a>, and you simply need to hold onto them to trade for free.<br><br><b>Get Started</b><ol><li>Connect your KeepKey to the <a class=platform-link ng-click="signup(\'whatsnew\', \'\')">Platform</a></li><li><a class=platform-link ng-click="signup(\'whatsnew\', \'https://portal.shapeshift.io/me/settings/account-verification\')">Sign In or Verify</a> Your Free ShapeShift Account to earn <a class=platform-link ng-click="signup(\'whatsnew\', \'https://shapeshift.com/fox-token\')">100 FOX</a> tokens</li><li>Trade Crypto for Free in the <a classs=platform-link ng-click="signup(\'whatsnew\', \'https://beta.shapeshift.com/\')">ShapeShift Platform</a></li></ol><br><br></div></div>'),
      e.put('app/popup/walletlist/walletlist.tpl.html', '<section id=wallet-list ng-controller=WalletListController><div ng-hide=loaded><ng-include src="\'app/popup/walletlist/loading.tpl.html\'"></ng-include></div><div ng-show=overlayActive><ng-include src="\'app/popup/walletlist/overlay.tpl.html\'"></ng-include></div><div class=wallet-fade ng-hide=!showWalletList()><ng-include src="\'app/popup/walletlist/header.tpl.html\'"></ng-include><ng-include src="\'app/popup/walletlist/wallets.tpl.html\'"></ng-include><div class=button-footer><div class=left><strong><device-label></device-label></strong></div><div class=textButton><button class="fa fa-plus footer-button" ng-click="go(\'/accountConfig\', \'slideLeft\')" uib-tooltip="Add Account" tooltip-popup-delay=500 tooltip-placement="auto top"></button><div>Add Account</div></div><div class=textButton><button class="fa fa-exchange footer-button ss-button" ng-click="go(\'/exchange\', \'slideLeft\')" uib-tooltip=ShapeShift tooltip-popup-delay=500 tooltip-placement="auto top"></button><div>Trade</div></div></div></div><notification></notification><notification-message can-notify=showWalletList()></notification-message></section>'),
