...
angular.module('kkWallet', ['ngRoute', 'ngAnimate', 'ui.bootstrap', 'monospaced.qrcode', 'kkCommon', 'ngMessages', 'ngSanitize']).config(['$compileProvider', function(e)
  ...
  angular.module('kkWallet').controller('ReceiveController', ['$rootScope', '$scope', '$routeParams', '$location', 'DeviceBridgeService', 'WalletNodeService', 'NavigationService', '$timeout', 'environmentConfig', 'ReceiveAddressService', 'PinLockService', function(e, t, n, a, o, i, c, s, r, l, pinLock)
  {
    ...
    var d = new Promise(function(resolve)
      {
        t.$on('ButtonRequest', function(e, n)
        {
          'ButtonRequest_Address' === n.code && (resolve(n.code), t.busy = false)
        })
      }),
      u = !1;
    t.walletId = n.walletId,
      ...
      t.$on(t.isSingleAddressAccount ? 'EthereumAddress' : 'Address', function(e, n)
      {
        console.assert(t.unusedAddress.address === (n.address_str || n.address),
                      'Address from KeepKey does not match!')
        t.confirmDevice = true
      }),
  ...
  angular.module('kkWallet').controller('ViewXPubController', ['$rootScope', '$scope', '$routeParams', '$location', 'DeviceBridgeService', 'WalletNodeService', 'NavigationService', 'XPubAddressService', 'PinLockService', function(e, t, n, a, o, i, c, l, pinLock)
  {
    ...
    var d = new Promise(function(resolve)
      ...
      t.$on('XPubAddress', function(e, n)
      {
        console.assert(t.unusedAddress.address === n.address,
                      'XPubAddress from KeepKey does not match!')
        t.confirmDevice =  true
      }),
      t.$on('$destroy', function()
      {
  ...
  angular.module('kkWallet').controller('SuccessController', ['$scope', '$routeParams', 'NavigationService', 'NotificationMessageService', 'WalletNodeService', function(e, t, n, notes)
  {
    e.ok = n.goToPrevious.bind(null, 'slideRight');
    e.message = notes.get() || decodeURIComponent(t.message),
    notes.clear(),
    n.setNextTransition('slideLeft')
  }]),
  angular.module('kkWallet').config(['DeviceBridgeServiceProvider', function(e)
  {
    ...
    e.when('disconnected', ['$injector', 'WalletNodeService', 'DeviceFeatureService', 'ShapeShiftAuthTokenService', 'SpotPriceService', 'PinLockService', function(e, n, a, o, spot, pinLock)
      ...
      e.when('ButtonRequest', ['$injector', 'PinLockService', function(e, pinLock)
      {
        ...
        if ('ButtonRequest_Address' === this.request.message.code) pinLock.reset()
      }]),
      ...
      e.when('Address', function() {}),
      e.when('EthereumAddress', function() {}),
      e.when('XPubAddress', function() {}),
  ...
  {
    e.put('app/popup/about/about.tpl.html', '<section id=about ng-controller=AboutController><header><back-button class="button-left header-button"></back-button></header><div class=content><h2><div class=applogo-icon><img src="/images/icon.png"><div class=applogo-features>About KeepKey<ul><li><span>Client:</span>v{{device.wallet_version}}</li><li><span>Firmware:</span>{{device.version}}</li><li><span>Bootloader:</span>{{device.bootloaderInfo.tag}}</li><li><span>Model:</span>{{device.model}}</li><li ng-hide=!device.firmware_variant><span>Variant:</span>{{device.firmware_variant}}</li></ul></div></div></h2><div class=features><ul><li><span class=shortwidth>Device ID:</span>{{device.device_id}}</li><li><span class=shortwidth>Revision:</span>{{device.revision}}</li><li ng-hide=!device.firmware_hash><span class=shortwidth>Firmware Hash:</span>{{device.firmware_hash}}</li><li><span class=shortwidth>Bootloader Hash:</span>{{device.bootloader_hash}}</li><li><span class=longwidth>Initialized:</span>{{device.initialized}}</li><li><span class=longwidth>Pin Protection:</span>{{device.pin_protection ? "enabled" : "disabled"}}</li><li><span class=longwidth>Passphrase Protection:</span>{{device.passphrase_protection ? "enabled" : "disabled"}}</li></ul></div><div class=menu><ul class=about-sections><li><button class=menu-item ng-click=launchWebsite($event)>Website</button></li><li><button class=menu-item ng-click=launchBlog($event)>Blog</button></li></ul></div><div class=social-media-links><ul><li><i class="fa fa-twitter" ng-click=launchTwitter($event)></i></li><li><i class="fa fa-facebook-f" ng-click=launchFacebook($event)></i></li><li><i class="fa fa-reddit" ng-click=launchReddit($event)></i></li><li><i class="fa fa-youtube" ng-click=launchYoutube($event)></i></li><li><i class="fa fa-github" ng-click=launchGithub($event)></i></li></ul></div></div><div class=footer><div class=left><strong><device-label></device-label></strong> connected</div></div><notification-message></notification-message></section>'),
      ...
      e.put('app/popup/directives/walletBalance.tpl.html', '<div class=wallet-balance ng-class="{\'has-pending-balance\': pending}"><div class=balance><span ng-show=loading><span class="fa fa-refresh fa-spin"></span></span><formatted-amount ng-hide=loading amount=balance currency=currency></formatted-amount><exchange-formatted-amount ng-hide=loading amount=balance currency=currency></exchange-formatted-amount></div><div class=unconfirmed-balance><span ng-show=showInstantSend><span class="fa fa-bolt instantsend-icon"></span><span class="instantsend-balance"><span>InstantSend:&nbsp;</span><formatted-amount signed=true amount=instantsend currency=currency></formatted-amount></span></span><span ng-show=showPending><span>Pending:&nbsp;</span><formatted-amount signed=true amount=pending currency=currency></formatted-amount></span></div></div>'),
