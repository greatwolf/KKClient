...
angular.module('kkTransactions', ['ngRoute', 'kkCommon', 'ui.bootstrap']).config(['$compileProvider', function(a)
  ...
  {
    a.put('app/transactions/transaction.tpl.html', '<div class=transaction-list ng-controller=TransactionListController><header><div class="logo upper-left-logo"></div><h1>{{wallet.name}} ({{wallet.coinType}})</h1><h5 ng-hide="wallet.accountNumber===undefined">Account #{{wallet.accountNumber}}</h5><div class=table><div class=thead><div class=date>Date</div><div class=time>Time</div><div class=address>Address</div><div class=amount>Amount</div><div class=amount>Network Fee</div><div class=link></div></div></div></header><section><div class=table><notification></notification><div class=transaction ng-class="{\'odd\': $odd, \'even\': $even}" ng-repeat="transaction in wallet.txHist | orderBy:\'-date\'"><div class=date>{{transaction.date | date:\'mediumDate\' }}</div><div class=time>{{transaction.date | date:\'hh:mm:ss a\'}}</div><div class=address><div ng-repeat="address in transaction.addresses track by $index">{{address}}</div></div><div class=amount><div class=received hide-if-zero=transaction.amountReceived>+&nbsp;<formatted-amount amount=transaction.amountReceived currency=wallet.coinType suppress-symbol=true></formatted-amount></div><div class=sent hide-if-zero=transaction.amountSent>-&nbsp;<formatted-amount amount=transaction.amountSent currency=wallet.coinType suppress-symbol=true></formatted-amount></div></div><div class=amount><div hide-if-zero=transaction.fee>-&nbsp;<formatted-amount amount=transaction.fee currency=wallet.coinType suppress-symbol=true></formatted-amount></div></div><div class=link><a href={{transaction.link}} target=_blank><span ng-show=!transaction.pending>details</span> <span ng-show=transaction.pending>unconfirmed</span></a></div></div></div></section></div>')
  }]);
